...
void cLuxScriptHandler::InitScriptFunctions()
{
	...
	AddFunc("void SetMessageExact(string &in message, float afTime)",(void *)SetMessageExact);
	...
	AddFunc("void CreateEntityAtFirstArea(string &in asEntityName, string &in asEntityFile, bool abFullGameSave)",(void *)CreateEntityAtFirstArea);
...
void __stdcall cLuxScriptHandler::SetMessageExact(string &message, float afTime)
{
    std::wstring wMessage(message.length(), L' ');
    std::copy(message.begin(), message.end(), wMessage.begin());

	gpBase->mpMessageHandler->SetMessage(wMessage, afTime);
}
