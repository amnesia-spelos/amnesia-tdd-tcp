...
The `avatarpose` line carries a crouch flag alongside the Pose. `CONTEXT.md` defines a Pose as position and facing only, so a spike that renders crouch should either widen that definition or keep naming the flag separately (a gap for `/domain-modeling`). Rendering it has two independent parts.
