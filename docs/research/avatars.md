...
- While the rendered Pose has the lantern raised, the handler fades in a shadowless point light that copies the local lantern's glow (`cAvatarLanternModel` in `AvatarLanternModel.h` picks the brightness). **Pitfall:** the glow is not `cLuxPlayerLantern`'s light, whose `Player_Lantern` entry in `config/game.cfg` has a radius of `0.0001` and no gobo. It is the widest `PointLight` in the hand-held lantern model (`models/hand_objects/lantern/lantern.ho` names `hand_lantern.ent`), whose light the hand object fades and flickers. `LoadLanternLight` reads its colour, radius, and gobo from there. It is placed at the standing camera height and turned by yaw only, so it ignores crouch and pitch.
