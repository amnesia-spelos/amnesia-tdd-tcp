...
$testDirectory = Join-Path $repositoryRoot 'tests\avatar-clip-model'
...
$executable = Join-Path $outputDirectory 'AvatarClipModelTests.exe'

& cl /nologo /EHsc /W4 /WX /I $sourceDirectory `
    (Join-Path $testDirectory 'AvatarClipModelTests.cpp') `
    (Join-Path $sourceDirectory 'AvatarClipModel.cpp') `
    /Fo:"$outputDirectory\" `
    /Fe:$executable
if ($LASTEXITCODE -ne 0) {
    throw "Avatar clip model harness compilation failed with exit code $LASTEXITCODE."
}

& $executable
if ($LASTEXITCODE -ne 0) {
    throw "Avatar clip model test failed with exit code $LASTEXITCODE."
}
