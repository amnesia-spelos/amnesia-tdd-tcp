...
$testDirectory = Join-Path $repositoryRoot 'tests\avatar-lantern-model'
...
$executable = Join-Path $outputDirectory 'AvatarLanternModelTests.exe'

& cl /nologo /EHsc /W4 /WX /I $sourceDirectory `
    (Join-Path $testDirectory 'AvatarLanternModelTests.cpp') `
    (Join-Path $sourceDirectory 'AvatarLanternModel.cpp') `
    /Fo:"$outputDirectory\" `
    /Fe:$executable
if ($LASTEXITCODE -ne 0) {
    throw "Avatar lantern model harness compilation failed with exit code $LASTEXITCODE."
}

& $executable
if ($LASTEXITCODE -ne 0) {
    throw "Avatar lantern model test failed with exit code $LASTEXITCODE."
}
