...
$testDirectory = Join-Path $repositoryRoot 'tests\avatar-pose-model'
...
$executable = Join-Path $outputDirectory 'AvatarPoseModelTests.exe'

& cl /nologo /EHsc /W4 /WX /I $sourceDirectory `
    (Join-Path $testDirectory 'AvatarPoseModelTests.cpp') `
    (Join-Path $sourceDirectory 'AvatarPoseModel.cpp') `
    /Fo:"$outputDirectory\" `
    /Fe:$executable
if ($LASTEXITCODE -ne 0) {
    throw "Avatar Pose model harness compilation failed with exit code $LASTEXITCODE."
}

& $executable
if ($LASTEXITCODE -ne 0) {
    throw "Avatar Pose model test failed with exit code $LASTEXITCODE."
}
